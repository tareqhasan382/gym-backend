...
const traineeSchedule = catchAsync(async (req: Request, res: Response) => {
  const user = req.user;
...
const allTrainee = catchAsync(async (req: Request, res: Response) => {
  try {
    const result = await UserModel.find({ role: "TRAINEE" });
...
export const traineeControoler = {
  createTrainee,
  traineeSchedule,
  allTrainee,
};
